package com.power.utils.tags.form.select;
...
public class SelectTag extends SimpleTagSupport{
	...
	public void doTag()throws JspException,IOException{
    	...
    	try {
    		...
    		if(org.springframework.util.StringUtils.hasText(nameKey)||(isJXdata!=null&&isJXdata)){
    			sb.append("<select ");
    			if(org.springframework.util.StringUtils.hasText(name)){
        			sb.append(" name='").append(name).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(clazz)){
    				sb.append(" class='").append(clazz).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(disabled)){
    				sb.append(" disabled='").append(disabled).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(id)){
    				sb.append(" id='").append(id).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(multiple)){
    				sb.append(" multiple='").append(multiple).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(style)){
    				sb.append(" style='").append(style).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(tabindex)){
    				sb.append(" tabindex='").append(tabindex).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(title)){
    				sb.append(" title='").append(title).append("' ");
    			}
    			if(org.springframework.util.StringUtils.hasText(otherAttr)){
    			...
    			if(isJXdata!=null&&isJXdata){
    				childs= JsonUtils.getBeanListFromJsonData(data, LinkedCaseInsensitiveMap.class);
    			}else{
    				...
    				if(isgetChildChild!=null&&isgetChildChild&&childs!=null&&childs.size()>0){
    					...
    					for(Map<String, Object> cc:childs){
    						cclist=(List<Map<String, Object>>) cc.get("childs");
        		...
        		if(childs!=null&&childs.size()>0){
        			for (Map<String,Object> option : childs) {
        				...
        				Object value=option.get("ab_st_name");
        	            if(org.springframework.util.StringUtils.hasText(initSelectedKey)&&initSelectedKey.equals(key)){
