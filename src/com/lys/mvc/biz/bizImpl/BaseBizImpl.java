package com.lys.mvc.biz.bizImpl;
		...
		if(list!=null&&list.size()>0){
			...
			try {
				for(BizTransUtil biztrans : list){
					...
					//如果是自定义sql
					if(org.springframework.util.StringUtils.hasText(biztrans.getExecute_sql())){
						executenum=baseDao.execute(biztrans.getExecute_sql(), biztrans.getExecute_params());
					}
					//如果是新增
					else if(CommonUtil.INSERT.equals(biztrans.getExecuteType())){
					...
					//如果是删除
					else if(CommonUtil.DELETE.equals(biztrans.getExecuteType())){
					...
					//如果是修改
					else if(CommonUtil.UPDATE.equals(biztrans.getExecuteType())){
					...
					biztrans.setReturnType(CommonUtil.RETURN_1);
					a++;
				}
				if(a==list.size()){
					return true;
				...
				SafetyFilter.logger.error("执行List<BizTransUtil>集合中的第："+a+"个对象出现错误，请仔细检查。");
				SafetyFilter.logger.error("对象是：list.get("+a+")==："+list.get(a).toString());
				SafetyFilter.logger.error("错误原因："+e.getMessage());
