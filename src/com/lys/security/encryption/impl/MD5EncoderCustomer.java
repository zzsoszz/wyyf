...
public class MD5EncoderCustomer implements Encoder {
	...
	private void md5Init() {
		count[0] = 0L;
		count[1] = 0L;
		state[0] = 0x67452301L;	// Load magic initialization constants.
		state[1] = 0xefcdab89L;
		state[2] = 0x98badcfeL;
		state[3] = 0x10325476L;
		return;
	...
	private void md5Update(byte[] inbuf, int inputLen) {
		...
		if ((count[0] += (inputLen << 3)) < (inputLen << 3))// Update number of bits
			count[1]++;
		count[1] += (inputLen >>> 29);
		partLen = 64 - index;
		...
		md5Memcpy(buffer, inbuf, index, i, inputLen - i);// Buffer remaining input
	}
	private void md5Final() {
		byte[] bits = new byte[8];
		int index, padLen;
		Encode(bits, count, 8);// Save number of bits
		index = (int) (count[0] >>> 3) & 0x3f;// Pad out to 56 mod 64.
