...
public class BizTransUtil { 
	...
	public BizTransUtil(Object add_obj){
		...
		this.executeType=CommonUtil.INSERT;
	}
	/**
	 * 操作对象
	...
	public BizTransUtil(Object del_id,Class<?> del_obj){
		...
		this.executeType=CommonUtil.DELETE;
	}
	/**
	 * 操作执行SQL---该方法待测试
	 * 删除---根据主键ID删除
	...
	public BizTransUtil(Map<String, Object> map,Class<?> obj,String executeType){
		if(CommonUtil.DELETE.equals(executeType)){
			this.del_map=map;
			this.del_obj=obj;
		}else if(CommonUtil.UPDATE.equals(executeType)){
			this.upd_map=map;
			this.upd_obj=obj;
		}
		this.executeType=executeType;
