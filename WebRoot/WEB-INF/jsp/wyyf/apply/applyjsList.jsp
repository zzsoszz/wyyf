	...
	<div class="modal-dialog modal-wide">
		<div class="modal-content">
			...
			<form id="addForm" action="#" class="form-horizontal" encType="multipart/form-data" method="post">
				...
				<div class="form-body">
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="form-group">
								<label class="control-label col-md-4">是否支付</label>
								<div class="col-md-8">
								
									<input name="payState" class="form-control" readonly="readonly"  />
								</div>
							</div>
						</div>
					</div> 
			...
			<div class="modal-footer">
				<button type="button" id="saveBtn"  class="btn blue" >保存结果图</button>
...
<script>
	...
	function loadotherPage(params) {
		...
		var url="wyyf/apply/jsapplyList";
	...
	function findapply(obj){
		...
		var url = "wyyf/apply/findjsApplyById";
		newMask();
	...
	function delapply(obj){
		bootbox.confirm('确定是否删除？',function(rs){
			if(rs){
				newMask();
				var advertisementId=$(obj).parents("tr").first().find("input[type='checkbox'].checkboxes").val();
				var url = "wyyf/apply/jsapplyDelete";
				$.post(url,"id="+advertisementId,function callback(data){  
					delMask();
					var resp = eval("("+data+")"); 
					if(resp.success){
