(function() {
	var version = "2.0.6", hasOwn = {}.hasOwnProperty, PingppSDK = function() {
	}, cfg = {
	...
	PingppSDK.prototype = {
		...
		createPayment : function(charge_json, callback, signature, debug) {
			...
			if (typeof charge_json == "string") {
				try {
					...
					this
							._innerCallback("fail", this
									._error("json_decode_fail"));
					return;
				}
			} else {
				charge = charge_json;
			...
			if (!hasOwn.call(charge, 'id')) {
				this._innerCallback("fail", this._error("invalid_charge",
						"no_charge_id"));
				return;
			}
			...
			if (!charge['credential']) {
				this._innerCallback("fail", this._error("invalid_credential",
			...
			if (channel == channels.upmp_wap) { // 调起银联支付控件，客户端需要安装银联支付控件才能调起
				...
				if (!hasOwn.call(credential, 'url')) {
					this._innerCallback("fail", this._error(
							"invalid_credential", "missing_field:url"));
					return;
				}
				location.href = credential['url'] + '?'
						+ stringify_data(credential, channel);
			} else if (channel == channels.yeepay_wap) {
				var fields = [ "merchantaccount", "encryptkey", "data" ];
				for ( var k in fields) {
					if (typeof fields[k] !== "function"
							&& !hasOwn.call(credential, fields[k])) {
						this._innerCallback("fail", this._error(
								"invalid_credential", "missing_field_"
										+ fields[k]));
						return;
					}
				}
				if (hasOwn.call(credential, "mode")
						&& credential["mode"] == "test") {
					location.href = cfg.YEEPAY_WAP_TEST_URL + '?'
							+ stringify_data(credential, channel, true);
				} else {
					location.href = cfg.YEEPAY_WAP_URL + '?'
							+ stringify_data(credential, channel, true);
				}
			} else if (channel == channels.wx_pub) {
				var fields = [ "appId", "timeStamp", "nonceStr", "package",
						"signType", "paySign" ];
				for ( var k in fields) {
					if (typeof fields[k] !== "function"
							&& !hasOwn.call(credential, fields[k])) {
			...
			}else if(channel==channels.alipay){
						...
						.invoke(
								...
								function(res) {
									if (res.err_msg == 'get_brand_wcpay_request:ok') {
										...
										self._innerCallback("fail", self
												._error("wx_result_fail",
														res.err_msg));
									}
								});
			}
		...
		_callpay : function() {
			...
			if (typeof wx != "undefined"
					&& typeof self._signature != "undefined") {
					...
					wx.chooseWXPay({
						...
						success : function(res) {
							if (res.errMsg == "chooseWXPay:ok") {
								...
								self._innerCallback("fail", self._error(
										"wx_result_fail", res.errMsg));
							}
						},
						cancel : function(res) {
							self._innerCallback("cancel");
				...
				wx.error(function(res) {
					wxConfigFailed = true;
					self._innerCallback("fail", self._error("wx_config_error",
							res.errMsg));
				});
				...
				if (document.addEventListener) {
					...
					document
							.attachEvent('onWeixinJSBridgeReady', eventCallback);
				}
			} else {
				this._jsApiCall();
			}
		...
		_testModeNotify : function(charge) {
			...
			if (charge['channel'] == channels.wx_pub) {
				...
				if (dopay) {
					...
					request.open('GET', cfg.PINGPP_NOTIFY_URL + charge['id']
							+ '?livemode=false', true);
					request.onload = function() {
						if (request.status >= 200 && request.status < 400
							...
							var extra = 'http_code:' + request.status
									+ ';response:' + request.responseText;
							self._innerCallback("fail", self._error(
									"testmode_notify_fail", extra));
						}
					};
				...
				if (hasOwn.call(charge, 'extra')) {
					params['extra'] = encodeURIComponent(JSON
							.stringify(charge['extra']));
				}
				location.href = cfg.PINGPP_MOCK_URL + '?'
						+ stringify_data(params);
			}
		}
	};
